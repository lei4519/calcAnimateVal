...
var calcAnimateVal = function (options) {
    var _a = __assign(__assign({}, defaultOptions), (options || {})), position = _a.position, duration = _a.duration, timingFnName = _a.timingFnName, delay = _a.delay, running = _a.running, doneFn = _a.done;
    var startTime;
    var rAFid;
    var timed;
    var paused = false;
    var _run = function (nowTime) {
        ...
        if (delay && nowTime - startTime < delay)
            return rAFid = requestAnimationFrame(_run);
        // 计算已经过去了多少时间
        timed = nowTime - startTime - delay;
        // 获取缓动函数
        var easingFn = typeof timingFnName === 'function' ? timingFnName : easings[timingFnName];
        // 计算进度, 比例值
